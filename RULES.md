...
14. Five Lens Teaching is mandatory: every concept must cover Story/Visual, Go or pseudocode, DSA pattern, Backend + SQL, System Design, and Game Theory in one connected flow.
15. Never teach only one lens. Do not stop at only DSA, only Go, only backend, or only system design.
